...
struct Bess {
	...
	double _ik0(const double x); // sqrt(x)*exp(x)*Integral[BesselK[x, inf]], x >= d;
	double k0(const double x); // BesselK[0,x], x >= d
	double ik0(const double x); // Integral BessK0[x, inf], x >= d
	double ik02(const double x); // Integral BessK0[x, inf], x < 2
	long double _ik02(const double x); // Integral BessK0[0, x], x < 2
	double analytic_ik0ab(const double x1, const double x2); // integral BessK0[x1, x2]
	double ik0ab(const double x1, const double x2);
